...
void WaterBox_PMU::_requestEvent() {
  ...
  //     break;
  //   case Req_Field:
  //     sprintf_P(_recBuffer, PSTR("%s"), F("OK"));
  //     break;
  // }
}
...
void WaterBox_PMU::_cmd(char *_str) {
  ...
  */
  ...
  memset(_CmdStr, '\0', 16);  // 清空緩衝BUFFER

  if (sscanf_P(_str, PSTR("TIME %d"), &_SleepSec) > 0) {  // TIME,30 設定睡眠時間30Sec
  ...
  if (sscanf_P(_str, PSTR("LOWPOWER %d"), &_PowerSave) > 0) {  // LOWPOWER,1 -> 開啟省電狀態
  ...
  if (strstr_P(_str, PSTR("CURRENT")) != NULL) {  // CURRENT -> 回傳目前電流mA
    _REQUEST = Req_BAT_CURRENT;
  }

  if (sscanf_P(_str, PSTR("%s %s"), _CmdType, _CmdStr) == 2) {
    if (strcmp_PF(_CmdType, PSTR("F1")) == 0)
      strcpy(Field_1, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F2")) == 0)
      strcpy(Field_2, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F3")) == 0)
      strcpy(Field_3, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F4")) == 0)
      strcpy(Field_4, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F5")) == 0)
      strcpy(Field_5, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F6")) == 0)
      strcpy(Field_6, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F7")) == 0)
      strcpy(Field_7, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F8")) == 0)
      strcpy(Field_8, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F9")) == 0)
      strcpy(Field_9, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F10")) == 0)
      strcpy(Field_10, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F11")) == 0)
      strcpy(Field_11, _CmdStr);

    if (strcmp_PF(_CmdType, PSTR("F12")) == 0)
      strcpy(Field_12, _CmdStr);

    if (strlen(_CmdType) <= 3) {
      _Deguber(_CmdStr, H_PMU, EOL);
      _REQUEST = Req_Field;
    }
  }
}
